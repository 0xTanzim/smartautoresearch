...
DO NOT MODIFY during the smartautoresearch loop — this is the read-only judge.

This file is a REFERENCE EXAMPLE (see references/example-prompt.md and
references/example-test-cases.json). It shows the eval-agent what a well-formed
deterministic eval.py looks like: pure-Python assertions, multi-signal proxy
heuristics for subjective checks, an ASSERTIONS list, per-output detail, an
assertion breakdown, and a final `METRIC pass_rate=X.XXXX` line. It is NOT wired
into any live loop — copy the shape, not the file.
