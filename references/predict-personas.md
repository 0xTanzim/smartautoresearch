...
## Grounding Personas with Research

When `predict --research` is used (see `commands/predict.md` Phase 1), personas receive dated, sourced findings from `commands/research.md` alongside the codebase knowledge base. This matters most for the Security Analyst (current CVEs, disclosed exploits) and the Performance Engineer (current benchmark data) — their default red flags are timeless, but "is this still the recommended approach" is not, and stale personal knowledge should defer to dated citations when the two conflict.
