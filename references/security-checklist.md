...
## Relationship to `scripts/orchestrate.sh screen-cmd`

`screen-cmd`'s deny-list (fork bombs, `rm -rf`, `curl|sh`, embedded credentials, force-push, destructive SQL) is a narrow, mechanical pre-execution gate — it stops the orchestrator from running an obviously catastrophic command. It is not a substitute for this checklist's actual security audit. A command can pass `screen-cmd` cleanly and still be part of a vulnerable system; `security` audits the system, `screen-cmd` only guards the orchestrator's own shell-out behavior.
