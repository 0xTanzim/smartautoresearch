---
name: smartautoresearch:predict
...
- `--research` — before persona analysis, dispatch `commands/research.md` for any claims needing external validation (market data, competitor behavior, current best practices) and feed findings to personas as grounded citations
- `--chain`, `--<subcommand>`
...
If `--research` is set, dispatch `commands/research.md` now with queries derived from the goal (e.g. "current best practice for X", "known CVEs affecting Y") and fold the dated findings into the knowledge base handed to personas below.
...
Each persona receives: task description + codebase knowledge (+ research findings if gathered) + their specific evaluation criteria.
...
Create output directory: `smartautoresearch/predict-{YYMMDD}-{HHMM}/`
...
Write handoff.json: version "1.0.0", source "predict", timestamp, status (COMPLETE|ERROR), findings = consensus findings with severity + confidence + file:line, config{scope, goal, depth}.
