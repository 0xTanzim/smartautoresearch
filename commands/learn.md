...
3. Create output directory: `smartautoresearch/learn-{YYMMDD}-{HHMM}/`
...
### Per-page contract
- `generated_by: smartautoresearch` in YAML frontmatter
...
- **Name collision:** before overwriting a page, check for `generated_by: smartautoresearch` frontmatter; if absent (user-created) skip with warning. `--force` overrides.
...
After completion, write handoff.json: version "1.0.0", source "learn", timestamp, status (COMPLETE|BOUNDED|USER_INTERRUPT|ERROR), results_tsv path, findings = documentation gaps remaining (each `{id?, type:"gap", severity?, file_line?, summary}` per `references/handoff-schema.md`), config{mode, scope, depth}.
