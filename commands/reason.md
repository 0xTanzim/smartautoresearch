...
3. Create output directory: `smartautoresearch/reason-{YYMMDD}-{HHMM}/`
...
After completion, write handoff.json: version "1.0.0", source "reason", timestamp, status (COMPLETE|CONVERGED|USER_INTERRUPT|BOUNDED|ERROR), results_tsv path, findings = [{id, type: "recommendation", summary: winner description}], config{task, domain, mode}.
