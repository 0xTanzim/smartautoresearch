---
name: smartautoresearch:scenario
...
2. Create output directory: `smartautoresearch/scenario-{YYMMDD}-{HHMM}/`
...
After completion, write handoff.json: version "1.0.0", source "scenario", timestamp, status (COMPLETE|SATURATED|BOUNDED|USER_INTERRUPT|ERROR), results_tsv path, findings = scenarios by severity (each `{id?, type:"scenario", severity, file_line?, summary}` per `references/handoff-schema.md`), config{scenario, domain, scope}.
