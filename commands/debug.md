...
2. Create output directory: `smartautoresearch/debug-{YYMMDD}-{HHMM}/`
...
After completion, write handoff.json to output directory: version "1.0.0", source "debug", timestamp, status (COMPLETE|USER_INTERRUPT|BOUNDED|ERROR), results_tsv path, findings = confirmed bugs with severity + file:line, config{scope, symptom}.
