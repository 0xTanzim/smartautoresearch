---
name: smartautoresearch:improve
...
- research → whichever category matches the research goal's topic
- Override with `--seeds`.
...
1. Learn summary (`smartautoresearch/learn-*/summary.md`, most recent) → read it
...
6. If nothing found → warn: "No product context. Run `$smartautoresearch learn --mode summarize` for better results."

## Phase 2: Research Loop

Create output directory: `smartautoresearch/improve-{YYMMDD}-{HHMM}/`
TSV header: `# metric_direction: higher_is_better`
Columns: `iteration|timestamp|category|research_question|status|source|insight_problem|insight_mechanism|confidence|classification|search_date`
...
- Per iteration: form research question → dispatch to `commands/research.md` for parallel web search (do NOT do a single ad-hoc WebSearch call here — always route through research.md so results are date-stamped and multi-query) → synthesize → normalize to canonical insight schema → classify (new/extension/duplicate) → tag confidence (HIGH: 3+ sources, MEDIUM: 2, LOW: 1) → cross-check against codebase → log with `search_date`
...
**Insight schema:** `{problem: 10-word canonical form, affected_persona: ICP segment, proposed_mechanism: how to address, expected_outcome: what success looks like, search_date: when the supporting research was gathered}`
...
4. **2-sentence rationale** per item citing research evidence (with source + search_date)
...
- Top disclaimer: "Auto-generated from research findings gathered on the dates cited below. DECISION NEEDED items and LOW-confidence sections require your judgment."
- Problem statement (from research evidence chain, with search_date per citation)
...
Write `research-findings.md` — all insights with citations + confidence + search_date.
...
Write `handoff.json`: version "1.0.0", source "improve", timestamp, status (COMPLETE|SATURATED|USER_INTERRUPT|BOUNDED|ERROR), results_tsv path, findings = improvements with tier + confidence + prd_path, config{goal, icp, depth, categories_explored, insights_total, prds_generated}.
