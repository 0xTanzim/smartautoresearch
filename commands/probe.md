---
name: smartautoresearch:probe
description: "8 personas interrogate requirements until constraints saturate"
argument-hint: "[Topic: <text>] [Scope: <glob>] [--depth shallow|standard|deep] [--personas N] [--mode interactive|autonomous] [Iterations: N] [--evals] [--from-drift]"
---
...
- `--from-drift` — this probe was triggered by the orchestrator's requirements-drift signal, not a fresh user request. When set, seed the constraint registry with the STALE predicate/config from `orchestrator-state.json` and explicitly ask each persona "does this constraint still hold?" in round 1, rather than starting from zero.
...
- Initialize constraint registry — empty, UNLESS `--from-drift`, in which case seed it with the stale predicate/config being re-validated (see Parse Arguments above)
...
- On round 1 of a `--from-drift` run, every active persona's first question must be "does [stale constraint] still hold, or has it changed?" before generating net-new questions
...
- If `--from-drift`: mark each stale constraint as `confirmed` (unchanged), `revised` (changed — record old vs new), or `obsolete` (no longer applies)
...
Create output directory: `smartautoresearch/probe-{YYMMDD}-{HHMM}/`

1. Write `constraints.md` — full constraint registry organized by category
2. Write `conflicts.md` — unresolved contradictions
3. If `--from-drift`: write `drift-report.md` — confirmed/revised/obsolete breakdown, and whether the orchestrator's pinned predicate should be re-derived (hands off to `plan --from-drift`)
4. Generate ready-to-run config:
...
Write handoff.json: version "1.0.0", source "probe", timestamp, status (COMPLETE|SATURATED|USER_INTERRUPT|BOUNDED|ERROR), findings = constraints, config = derived config. If `--from-drift`, also set `drift_resolution` ∈ {confirmed_no_change, revised, obsolete} so the orchestrator's `requirements_drift` flag can be cleared or escalated to `plan`.
