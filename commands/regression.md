---
name: smartautoresearch:regression
...
Backed by `scripts/score-regression.sh verdict <results.tsv>` (exit 0 = STABLE; exit 1 = UNSTABLE **or** BASELINE_UNAVAILABLE — the two exit-1 verdicts are disambiguated by stdout, `UNSTABLE` vs `BASELINE_UNAVAILABLE`, never by the exit code alone) — this is the real implementation, not a placeholder; see `scripts/score-regression.sh` for the weighted-average math.
...
`smartautoresearch/regression-{YYMMDD}-{HHMM}/` → `regression-results.tsv`, `stability-report.md`, `dimensions/<dim>.md`, `baseline/`, `evals-summary.md` (if `--evals`), `handoff.json`.
...
Write `handoff.json` to the output directory: version "1.0.0", source "regression", timestamp,
...
Verify-command screen via `scripts/orchestrate.sh screen-cmd` (no `rm -rf` / `curl|sh` / fork bombs / embedded credentials); worktree cleanup + prune on crash; data-migration refuses any non-allowlisted DB URL; probe auto-skips non-interactively; chained `ship` never auto-deploys.
