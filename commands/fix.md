...
3. Create output directory: `smartautoresearch/fix-{YYMMDD}-{HHMM}/`
...
After completion, write handoff.json to output directory: version "1.0.0", source "fix", timestamp, status (COMPLETE|USER_INTERRUPT|BOUNDED|ERROR), results_tsv path, findings = unfixed errors, config{target, scope, guard}.
