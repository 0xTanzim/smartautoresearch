...
**REQUIRES EXPLICIT USER APPROVAL** (unless --auto with zero errors). This is a high-blast-radius, often-irreversible action (deploy, publish, push) — never bypass this gate regardless of autonomy mode.
...
Create output directory: `smartautoresearch/ship-{YYMMDD}-{HHMM}/`
Write:
...
Write handoff.json: version "1.0.0", source "ship", timestamp, status (COMPLETE|DRY_RUN|ROLLBACK|ERROR), findings = blockers/warnings found during prep.
