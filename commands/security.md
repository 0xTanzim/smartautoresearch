---
name: smartautoresearch:security
...
Create output directory: `smartautoresearch/security-{YYMMDD}-{HHMM}/`
...
After completion, write handoff.json to output directory: version "1.0.0", source "security", timestamp, status (COMPLETE|USER_INTERRUPT|BOUNDED|ERROR), results_tsv path, findings = all findings with severity + OWASP + STRIDE + file:line, config{scope, focus, depth}.
